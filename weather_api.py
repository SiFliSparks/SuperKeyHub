...
class QWeatherAPI:
    ...
    def _get_city_id(self, city: str) -> str:
        ...
        if self.api_host != "https://devapi.qweather.com":
            paths: list[str] = ["/v2/city/lookup", "/city/lookup", "/geo/city/lookup"]
            for path in paths:
        ...
        try:
            params: dict[str, str] = self._get_request_params({"location": city})
    ...
    def _fetch_current_weather(self, city: str) -> dict[str, Any] | None:
        ...
        try:
            params: dict[str, str] = self._get_request_params({"location": city_id})
            headers: dict[str, str] = self._get_request_headers()
            response: requests.Response = requests.get(
                f"{self.base_url}/weather/now",
                params=params,
    ...
    def get_weather_data(
        self, city: str | None = None, force_refresh: bool = False
    ) -> dict[str, Any]:
        if city is None:
        ...
        weather_data: dict[str, Any] | None = self._fetch_current_weather(city)
        forecast_data: list[dict[str, Any]] | None = self._fetch_weather_forecast(
            city, 3
        )
    ...
    def get_formatted_data(
        ...
        def get_weather_quality(
            temp: int, humidity: int, vis: float, pressure: float
        ) -> str:
            score: int = 100
    ...
    def _calculate_comfort_index(
        self, temp: int, humidity: int, wind_speed: int
    ) -> str:
